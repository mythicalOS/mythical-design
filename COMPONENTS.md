...
| `banner` | v2 | shipped — PACKAGED | — (no card yet) | **@mythicalos/preact-ui** `Banner` · react-ui mirror (tone + required glyph from **@mythicalos/ui-core** `BANNER_ICON`) | preact-ui 0.5.0 — rule #7: the glyph always rides along. v2 (ui-core 0.6.0, ported upstream from the Sága mockup rewire): non-status SURFACE variants `accent` (emphasis — accent-soft fill, accent-strong ink) + `neutral` (quiet notice — surface-hover fill); default glyphs ◆/○, consumers usually override with product SVG via `glyph` |
