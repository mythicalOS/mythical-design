...
| `theme-toggle` | v1 | shipped — PACKAGED | `ds/components-theme-toggle.html` | **@mythicalos/preact-ui** `ThemeToggle`, `ThemeToggleIcon`, `ThemeToggleSwitch` · react-ui mirror (mode logic in **@mythicalos/ui-core** `theme-toggle`: `ThemeMode`, `resolveThemeIsDark`, class derivations) | — three members, one component: segmented is the default with System first-class; icon shows the theme you're in (sun on light) and skips System; switch only under an explicit "Dark mode" label. Controlled only — persistence stays with the app. Terminal panes never follow (rule 3; `--my-shadow-knob` is a documented terminal residual). **Open (maintainer):** the card draws segmented track/options and switch track at `--my-r-pill` while rule 10 squares clickable controls at `--my-r-control` — the port follows the card |
