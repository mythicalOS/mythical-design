...
| `theme-toggle` | v1 | NEW — port in flight | `ds/components-theme-toggle.html` | targeted: **@mythicalos/preact-ui** `ThemeToggle` (segmented · icon · switch members) · react-ui mirror (mode logic in **@mythicalos/ui-core**) | — three members, one component: segmented is the default with System first-class; icon cross-fades sun↔moon and skips System; switch only under an explicit "Dark mode" label. Terminal panes never follow (rule 3) |
