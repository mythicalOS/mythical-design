...
- *(v0.5.19)* memory `#D79DF0` — the bookkeeping row's label hue. Minted, not mixed, and it
  could not have been mixed: an oklab `color-mix` is a convex combination, so every mix's
  `b` is at least the smallest `b` among its operands; every *other* terminal token has
  `b ≥ -0.0187` (dim's, the smallest) while this one is `-0.0914`, outside the hull of the
  seven that preceded it. Min oklab ΔE 0.141 to any other terminal token; 9.29:1 on the
  terminal bg
...
a colour outside them. (Careful, twice over: "outside them" is about position, not hue — a
mix CAN land on an unoccupied *hue*, because near the neutral axis every direction is
available; what it cannot do is get there with usable chroma. And the hull argument is
always relative to the tokens being mixed, never to a set that already contains the target.)
...
spec → implementation per product and defines how to point a coding agent at one.
