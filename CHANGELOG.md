...
- **No token values change.** Corrects the rationale comment on `--my-term-memory` and the matching
  ...
  exactly 92.5% the mix lands at 315.13°, and 315.34° needs 92.469%.
...
- **Review hardening**: `check-ds.sh` gains an `@dsInline`
...
- **Further hardening**: marker CLOSERS are now located
...
`mythical-select` CSS delivery is CSP-safe (first-consumer finding).
...
- Component registry (`COMPONENTS.md`) — 19 component rows with spec anchors,
  per-product implementation pointers, and the coding-agent prompt skeleton.
